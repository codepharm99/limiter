...
function AuthGate({ children }: { children: ReactNode }) {
  ...
  // Stage 2 hidden: ensureSession auto-creates the anonymous session, so this
  // branch only triggers when Supabase itself is unreachable.
  if (!hasSession && isStage2Enabled()) return <Navigate to="/auth" replace />
  return <>{children}</>
}
...
export default function App() {
  ...
  return (
    <Routes>
      ...
      {stage2 && <Route path="/auth" element={<AuthPage />} />}
